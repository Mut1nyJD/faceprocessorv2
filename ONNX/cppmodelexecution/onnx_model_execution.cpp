...
static BYTE REDLABEL[] = { 0, 0,   0,   255, 0,   255, 255, 255, 128, 255,0,  0,  128,0 };
static BYTE GREENLABEL[] = { 0, 255, 0,   0,   64, 255, 0,   255, 128, 192,128,128,0,  64 };
static BYTE BLUELABEL[] = { 0, 0,   255, 0,   64, 0,   255, 255, 128, 192,128,0,  128,64 };
...
//	OrtSessionOptionsAppendExecutionProvider_CUDA(session_options, 0);
	...
	FILE* fp = fopen("YOURIMAGE.jpg", "rb");

	if (fp != NULL)
	{
		// overwrite input_node_dims[0] with the actual batch-size as this is -1 which means any
		input_node_dims[0] = 1;
		...
		if (is_tensor)
		{
			...
			g_ort->GetDimensions(tshapeType, output_node_dims.data(), num_dims);
			for (size_t j = 0; j < num_dims; j++)
				printf("Output : dim %zu=%jd\n", j, output_node_dims[j]);
...
#pragma omp parallel for
			std::vector<float> predVal;
			predVal.resize(output_node_dims[1]);
			for (int y = 0; y < output_node_dims[2]; y++)
			{
				...
				for (int x = 0; x < output_node_dims[3]; x++)
				{
					...
					for (int k = 0; k < output_node_dims[1]; k++)
						predVal[k] = dataOut[x + y * output_node_dims[3] + k * output_node_dims[2] * output_node_dims[3]];
					float* softMaxed = Softmax(predVal.data(), output_node_dims[1]);
					float maxVal = softMaxed[0];
					unsigned char maxK = 0;
					for (int k = 1; k < output_node_dims[1]; k++)
					{
						if (softMaxed[k] > maxVal)
						{
							maxVal = softMaxed[k];
							maxK = k;
						}
					}
					bits[FI_RGBA_RED] = REDLABEL[maxK];
					bits[FI_RGBA_GREEN] = GREENLABEL[maxK];
					bits[FI_RGBA_BLUE] = BLUELABEL[maxK];
					bits += 3;
				}
			}
